...
client.addListener("messageCreate", (message) => {
  ...
  if (message.content.match(/i (have|need|want|am going) to (pee|piss)/gim)?.length > 0) {
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "chance") {
    ...
    if (interaction.options.getNumber("chance")) {
      // TEMPORARY
      ...
      config.chance = interaction.options.getNumber("chance");
      updateConfig();
...
function counting(msg) {
  ...
  if (
    ...
    msg.react("❌");
    return msg.reply("Not your turn dumbass");
  }
  ...
  if (config.countingChannel.count + 1 === parseInt(msg.content)) {
    ...
    if (config.countingChannel.count === config.countingChannel.goal) {
      return msg.reply(`Yay, you reached ${config.countingChannel.goal}!`);
    }
    updateConfig();
    return;
  } else {
...
add counting slash commands config
    make counting messups reset evry day
